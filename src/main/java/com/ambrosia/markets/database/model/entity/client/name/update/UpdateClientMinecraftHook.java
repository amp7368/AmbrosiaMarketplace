...
public class UpdateClientMinecraftHook {
    ...
    public static Future<Void> minecraftUpdate(DClientNameMeta client) {
    ...
    private static void updateMinecraft(DClientNameMeta client, ClientMinecraftDetails minecraft) {
        try (Transaction transaction = DB.beginTransaction()) {
            client.refresh();
            boolean isNewName = client.getMinecraft().isNewName(minecraft);
            client.setMinecraft(minecraft);
            if (isNewName) {
                DClientNameHistory lastName = client.getNameNow(NameHistoryType.MINECRAFT);
                DClientNameHistory newName = NameHistoryType.MINECRAFT.updateName(client, lastName, transaction);
